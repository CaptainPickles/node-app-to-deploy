...
mongoose.connect(`mongodb+srv://${process.env.mongo_id}:${process.env.mongo_password}@hitema-nest-test.y4zdm.mongodb.net/nodetest?retryWrites=true&w=majority`)
...
function authenticationToken(req, res, next) {
    ...
    try {
        ...
        console.log({ "token": token, userInToken: userInToken })
...
app.get('/mytask', [authenticationToken], async (req, res) => {
    const taches = await Task.find({ creePar: req.user.id });
    //const taches = Taches.getAll();
    res.status(200).send(taches);
});
...
app.post('/register', async (req, res) => {
    ...
    console.log("usercreated", userCreated._id)
